...
//			BasicCookieStore cookieStore = new BasicCookieStore();
			CloseableHttpClient hc = HttpClients.custom().setProxy(new HttpHost("10.6.30.135", 8080))
					.setDefaultCookieStore(new BasicCookieStore()).build();
			List<Cookie> cs = JsonProcessor.getInstance().readJdCookies();
			...
			JdLogin.showResponseInfo(response);
			hc.close();
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public static void main(String[] args) {
		JdOrderList order = new JdOrderList();
